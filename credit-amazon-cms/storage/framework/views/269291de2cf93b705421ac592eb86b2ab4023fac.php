    ...
    makeTable = function(items, q) {
        ...
        Object.keys(items).map(function(site){
            ...
            for(var i = 0, l = items[site].length; i < l; i++ ) {
                ...
                html += `<tr><td><a target="_blank" href="https://${site}/index.php?route=product/product&product_id=${item.id}">${item.name}</a></td><td>${item.quantity}</td> <td class="statuses" data-id="${item.id}" data-site=${site} id="status_${item.id}"></td> <td>${site}</td></tr>`;
            }
        });
    ...
    updateStatusListeners = function() {
        ...
        $('.statuses').on('mouseover', function(){
            ...
            getStatuses('<?php echo e(route('reports.byday.statuses')); ?>', function(response){
                ...
                if(response.bad_price) {
                    $('#status_' + response.item_id).css('background', '#ef8484');
                }
                console.log(data);
            }, data);
        });
